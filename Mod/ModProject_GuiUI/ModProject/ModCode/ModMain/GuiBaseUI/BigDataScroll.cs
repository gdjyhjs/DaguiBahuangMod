...
namespace GuiBaseUI
{
    public class BigDataScroll : MonoBehaviour
    {
        ...
        private Action<ItemCell, int> m_funcSetData;
        public Action<ItemCell, int> funcSetData { get { return m_funcSetData; } }
        ...
        public void Init(ScrollRect _scrollRect, ItemCell _itemcell, Action<ItemCell, int> _handleSetData, float wHeight = 0)
        {
            ...
            scrollRect.onValueChanged.AddListener(new Action<Vector2>(OnValueChanged));
            var topGo = new GameObject("top");
            var btmGo = new GameObject("button");
            top = topGo.AddComponent<RectTransform>();
            btm = btmGo.AddComponent<RectTransform>();
        ...
        class ItemPool
        {
            ...
            Transform poolRoot;
            public Dictionary<Action<ItemCell, int>, List<GameObject>> pool = new Dictionary<Action<ItemCell, int>, List<GameObject>>();

            public GameObject GetItem(Action<ItemCell, int> func, Transform parent)
            {
            ...
            public void PutItem(Action<ItemCell, int> func, GameObject item)
            {
                if (pool.ContainsKey(func))
                {
                    var v = pool[func];
            ...
            public void SetItem(Action<ItemCell, int> func, GameObject prefab)
            {
