...
namespace SchoolAdmin
{
    ...
    public class UISchoolRecruit
    {
        ...
        public UISchoolRecruit(UISchoolMainLobby ui)
        {
            ...
            btnSchoolAdmin.GetComponent<RectTransform>().anchoredPosition = new Vector2(btnSchoolAdmin.GetComponent<RectTransform>().anchoredPosition.x, btnSchoolAdmin.GetComponent<RectTransform>().anchoredPosition.y - 100);
            btnSchoolAdmin.GetComponent<RectTransform>().sizeDelta = new Vector2(btnSchoolAdmin.GetComponent<RectTransform>().sizeDelta.x + 50, btnSchoolAdmin.GetComponent<RectTransform>().sizeDelta.y);
            btnSchoolAdmin.transform.GetComponentInChildren<Text>().text = GameTool.LS("schoolAdmin_zhaohuanguanjia");
            ...
            { // 显示无需求的NPC
                ...
                go3 = CreateUI.NewText(GameTool.LS("schoolAdmin_showNpcNoDemand"));
                go3.transform.SetParent(go1.transform, false);
                go3.GetComponent<RectTransform>().sizeDelta = new Vector2(300, 30);
            ...
            { // 显示不合符入宗条件的NPC
                ...
                go2 = CreateUI.NewImage(SpriteTool.GetSprite("Common", "xianshigou"));
                go2.transform.SetParent(go1.transform, false);
                go3 = CreateUI.NewText(GameTool.LS("schoolAdmin_showAllNpc"));
                go3.transform.SetParent(go1.transform, false);
                go3.GetComponent<RectTransform>().sizeDelta = new Vector2(400, 30);
                go3.GetComponent<RectTransform>().anchoredPosition = new Vector2(15 + go3.GetComponent<RectTransform>().sizeDelta.x / 2, 0);
                go3.GetComponent<Text>().alignment = TextAnchor.MiddleLeft;
                go3.GetComponent<Text>().color = Color.black;
            ...
            { // 排序 sortType {名字，性别，境界}
                string[] sortName = new string[] { GameTool.LS("schoolAdmin_defaultSort"), GameTool.LS("schoolAdmin_nameSort"), GameTool.LS("schoolAdmin_genderSort"), GameTool.LS("schoolAdmin_levelSort") };
                ...
                action = () =>
                {
                    if (go4 == null)
                    {
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            ...
                            goBtn.GetComponent<RectTransform>().sizeDelta = new Vector2(200, 30);
                            ...
                            goBtnText.GetComponent<RectTransform>().sizeDelta = new Vector2(200, 30);
        ...
        private void UpdateNpcList(Transform bg)
        {
            ...
            if (maxPage > 0)
            {
                ...
                GameObject go2 = GuiBaseUI.CreateUI.NewText(GameTool.LS("book_last"), new Vector2(100, 20));
                ...
                go2 = GuiBaseUI.CreateUI.NewText(GameTool.LS("book_next"), new Vector2(100, 20));
                go2.transform.SetParent(go.transform, false);
                go2.GetComponent<Text>().alignment = TextAnchor.MiddleCenter;
                go2.GetComponent<Text>().color = Color.black;
            }
        ...
        private void UpdateNpcData(Transform connect, float x, float y, WorldUnitBase unit)
        {
            ...
            GameObject go2 = GuiBaseUI.CreateUI.NewText(GameTool.LS("battle_chakan"), new Vector2(100, 20));
            go2.transform.SetParent(go.transform, false);
            go2.GetComponent<Text>().alignment = TextAnchor.MiddleCenter;
            go2.GetComponent<Text>().color = Color.black;
            ...
            action = () =>
            {
                ...
                Action onOKCall = () =>
                {
                        ...
                        if (isNeed)
                        {
                            ...
                            Action giveEndAction = () =>
                            {
                                if (unit.data.school != null)
                                {
                                    ...
                                    int err1 = unit.CreateAction(new UnitActionSchoolExit());
                                    Console.WriteLine("退宗 " + err1);
                                    if (err1 != 0)
                                    {
                                        g.ui.OpenUI<UICheckPopup>(UIType.CheckPopup).InitData(GameTool.LS("common_tishi"), $"对不起，我在{school.name}中身居要职，暂时无法脱身！"+err1, 1);
                                        return;
                                    }
                                }
                                int err2 = unit.CreateAction(new UnitActionSchoolJoin(school.buildData.id, SchoolPostType.Out) { isTest = true }, true);
                                Console.WriteLine("入宗 " + err2);
                                if (err2 != 0)
                                {
                                    g.ui.OpenUI<UICheckPopup>(UIType.CheckPopup).InitData(GameTool.LS("common_tishi"), $"对不起，由于某些原因，我暂时无法加入贵宗！"+ err2, 1);
                                    return;
                                }
                                ...
                                // "我正需要此物，您送的太及时了，今后我就是{0}的人了，还请宗主大人多多关照！"
                                dramaDyn.dramaData.dialogueText[21802] = string.Format(GameTool.LS("schoolAdmin_giveDramaConnect"), school.name);
                    ...
                    if (!isOk)
                    {
                        ...
                        dramaDyn.dramaData.unit = unit;
                        // 我现在不需要{0}。
                        dramaDyn.dramaData.dialogueText[21802] = string.Format(GameTool.LS("schoolAdmin_dontNeed"), school.name);
                        dramaDyn.OpenUI();
                    }
                };
            ...
            go2 = GuiBaseUI.CreateUI.NewText(GameTool.LS("npcInfo_zhaona"), new Vector2(150, 20));
            go2.transform.SetParent(go.transform, false);
            go2.GetComponent<Text>().alignment = TextAnchor.MiddleCenter;
            go2.GetComponent<Text>().color = Color.black;
            ...
            {
                // $"打听到{0}当前所在位置{1}。"
                string str = string.Format(GameTool.LS("schoolAdmin_npcFound"), unit.data.unitData.propertyData.GetName(), unit.data.unitData.GetPoint().ToString());
                g.ui.OpenUI<UICheckPopup>(UIType.CheckPopup).InitData(GameTool.LS("common_tishi"), str, 1);
            };
            go = GuiBaseUI.CreateUI.NewButton(action);
            go.transform.SetParent(connect, false);
            go.GetComponent<RectTransform>().anchoredPosition = new Vector2(x, y);

            go2 = GuiBaseUI.CreateUI.NewText(GameTool.LS("schoolAdmin_found"), new Vector2(100, 20));
            go2.transform.SetParent(go.transform, false);
            go2.GetComponent<Text>().alignment = TextAnchor.MiddleCenter;
            go2.GetComponent<Text>().color = Color.black;


            string need = GameTool.LS("schoolAdmin_noDemand");
            if (unit.allTroubles.Count > 0)
            {
                foreach (UnitTroubleBase trouble in unit.allTroubles)
                {
                    try
                    {
                        switch (trouble.troubleBaseItem.id)
                        {
                            ...
                            case 10201:
                                ...
                                break;
                            case 10301:
                                need = trouble.GetTextValues()[0] + " " + GameTool.LS("playerInfo_fabao");
                                break;
                            case 10401:
                                need = trouble.GetTextValues()[0] + " " + GameTool.LS("playerInfo_zuoqi");
                                break;
                            case 10501:
                                need = trouble.GetTextValues()[0] + " " + GameTool.LS("playerInfo_jiezhi");
                                break;
                            case 10601:
                            case 10701:
                            case 10801:
            ...
            go = GuiBaseUI.CreateUI.NewText(GameTool.LS("schoolAdmin_needBadly") + need, new Vector2(800, 20));
            go.transform.SetParent(connect, false);
            go.GetComponent<RectTransform>().anchoredPosition = new Vector2(x, y);
            go.GetComponent<Text>().color = Color.black;
        }
