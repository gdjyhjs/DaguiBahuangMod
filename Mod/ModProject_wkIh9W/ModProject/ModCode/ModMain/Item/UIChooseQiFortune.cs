...
namespace MOD_wkIh9W.Item
{
    ...
    {
        public UIChooseQiFortune(IntPtr ptr) : base(ptr) { }
        ...
        public void Init(bool reRead)
        {
            ...
            {
                var items = g.conf.roleCreateFeature._allConfList.ToArray();
                List<ConfRoleCreateFeatureItem> list = new List<ConfRoleCreateFeatureItem>(items);
        ...
        void UpdateUI()
        {
            ...
            for (int i = pageIndex * pageShowCount; i < (pageIndex + 1) * pageShowCount; i++)
            {
                ...
                var name = CommonTool.SetTextColor(itemName, GameTool.LevelToColor(selectItem.level, 1)) + "(" + selectItem.id + ")";
                ...
                go.SetActive(true);
                go.AddComponent<UISkyTipEffect>().InitData(GameTool.LS(selectItem.tips));
            }
