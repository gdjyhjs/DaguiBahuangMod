...
namespace MOD_wkIh9W.Item
{
    ...
    {
        public UIChooseHaotianEyeSkill(IntPtr ptr) : base(ptr) { }

        public Action<string, string> call; // 参数字符， 昊天眼技能
        ...
        public List<int> allItems
        {
            get
            {
                ...
                var list = g.conf.godEyeSkills.allBossID;
                foreach (var item in list)
                {
                    var skills = g.conf.godEyeSkills.GetSkillsID(item);
                    allSkills.AddRange(skills.ToArray());
                }
                return allSkills;
            }
        }
        ...
        void Awake()
        {
            ...
            textTitle.text = "选择昊天眼技能";
            leftTitle.text = "可从下面选择昊天眼技能";
            rightTitle.text = "已选择的昊天眼技能";
        }


        public void InitData(UIDaguiToolItem toolItem, int index)
        {
            foreach (var skillID in allItems)
            {
                ConfGodEyeSkillsItem skillItem = g.conf.godEyeSkills.GetItem(skillID);
                var selectItem = skillItem;
                var name = GameTool.LS(skillItem.name);
