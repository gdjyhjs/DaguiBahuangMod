...
namespace MOD_wkIh9W.Item
{
    ...
    {
        public UIDesignateHeartTechnique(IntPtr ptr) : base(ptr) { }

        public Action<string, string> call; // 参数字符(心法soleid)
        ...
        public DataStruct<string, DataProps.MartialData>[] allItems
        {
            get
            {
                ...
                {
                    var martialData = item.value.data.To<DataProps.MartialData>();
                    if (martialData.martialType == MartialType.Ability)
                    {
                        list.Add(new DataStruct<string, DataProps.MartialData>(item.value.data.soleID, martialData));
                    }
                }
                return list.ToArray();
            }
        ...
        void Awake()
        {
            ...
            textTitle.text = "选择心法";
            leftTitle.text = "可从下面选择心法";
            rightTitle.text = "已选择的心法";
        }
