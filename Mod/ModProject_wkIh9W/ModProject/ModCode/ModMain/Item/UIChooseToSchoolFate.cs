...
namespace MOD_wkIh9W.Item
{
    ...
    {
        public UIChooseToSchoolFate(IntPtr ptr) : base(ptr) { }

        public Action<string, string> call; // 参数字符， 逆天改命id
        public ConfSchoolFateItem[] allItems
        {
            get
            {
                return g.conf.schoolFate._allConfList.ToArray();
            }
        }
        ...
        public void InitData(UIDaguiToolItem toolItem, int index)
        {
            ...
            {
                var selectItem = item;
                var conf = g.conf.roleCreateFeature.GetItem(selectItem.fateFeature);
                if (conf==null)
                {
                    continue;
                }
                var name = GameTool.LS(conf.name);
        ...
        public void AddTip(GameObject go, ConfSchoolFateItem selectItem)
        {
            ...
            uiEvent.onMouseEnterCall += (Action)(() => {
                ConfRoleCreateFeatureItem conf = g.conf.roleCreateFeature.GetItem(selectItem.fateFeature);
