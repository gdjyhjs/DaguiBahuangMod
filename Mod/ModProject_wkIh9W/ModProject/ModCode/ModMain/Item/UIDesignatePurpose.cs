...
namespace MOD_wkIh9W.Item
{
    ...
    {
        public UIDesignatePurpose(IntPtr ptr) : base(ptr) { }

        public Action<string, string> call; // 参数字符， 特性id
        ...
        void Awake()
        {
            ...
            textTitle.text = "选择特性";
            leftTitle.text = "可从下面选择特性";
            rightTitle.text = "已选择的特性";
        }
        ...
        public void InitData(UIDaguiToolItem toolItem, int index)
        {
            foreach (var item in allItems)
            {
                ...
                go.AddComponent<Button>().onClick.AddListener((Action)(() =>
                {
                    ...
                    {
                        UITipItem.AddTip("最多选择2个特性！");
                        return;
                    }
                    if (this.selectItem.Contains(selectItem))
                    {
                        UITipItem.AddTip("最多选择不同的特性！");
                    }
