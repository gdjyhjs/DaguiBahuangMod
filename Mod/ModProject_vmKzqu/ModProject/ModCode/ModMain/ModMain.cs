...
namespace MOD_FixMod
{
    ...
    public class ModMain
    {
        ...
        public void Init()
        {
			...
			if (harmony == null)
			{
				harmony = new HarmonyLib.Harmony("MOD_vmKzqu");
			}
			harmony.PatchAll(Assembly.GetExecutingAssembly());

            corUpdate = g.timer.Frame(new Action(OnUpdate), 1, true);
            ClassInjector.RegisterTypeInIl2Cpp<UIMono>();
        }
        ...
        private void OnUpdate()
        {
            if (Input.GetKeyDown(KeyCode.BackQuote))
            {
                if (uimono == null)
                {
                    uimono = g.game.gameObject.AddComponent<UIMono>();
                }
                uimono.open = !uimono.open;
            }
        }
    }
}
