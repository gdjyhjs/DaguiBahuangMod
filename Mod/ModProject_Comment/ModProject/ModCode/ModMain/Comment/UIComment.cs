...
namespace Comment
{
    public class UIComment
    {
        ...
        static float arrowAlpha = 1; // 箭头透明度
        ...
        public void Init(UIBase ui, int targetType, int targetId)
        {
            ...
            Action action = () =>
            {
                ...
                if (rtf.anchoredPosition != pos)
                {
                    posMoveTime += Mathf.Min(1, Time.deltaTime * 3);
                    rtf.anchoredPosition = Vector2.Lerp(posOld, pos, posMoveTime);
                }
            };
        ...
        private void InitBg()
        {
            try
            {
                ...
                Action onFram = () =>
                {
                    ...
                    {
                        canvasGroup.alpha = Mathf.Lerp(canvasGroup.alpha, arrowAlpha, Time.deltaTime * 10);
                    }
                };
        ...
        private void MoveBg(bool isClick = true)
        {
            if (isClick)
            {
                ...
                posOld = bg.GetComponent<RectTransform>().anchoredPosition;
        ...
        public void GetData()
        {
            CommentMain.Log($"获取类型 {targetType} {targetId}");
            isGetData = true;
            ...
            string url = @"http://www.yellowshange.com/guiguMod/comment/GetCommentList/List/0_" + targetType + "_" + targetId+".html";
            new HttpData().GetHttp(ui, url, (s) =>
            {
                if (s.StartsWith("["))
                {
                    CommentMain.Log("取得评论数据成功");
                    ...
                    CommentMain.Log("评论数量：" + items.Count);
                ...
                {
                    CommentMain.Log("不需要更新数据");
                    SortItems();
                    InitUI();
                }
                else
                {
                    CommentMain.Log("获取数据错误 " + s);
        ...
        void DestroyChilds(Transform tf)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    CommentMain.Log("err1:" + e.Message);
                }
            }
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    CommentMain.Log("err2:" + e.Message);
                }
            }
        }
        ...
        private void InitUI()
        {
            ...
            {
                InitScrollItem(ref newGo, ref newGroup, scrollRect, items);
            }
            ...
            //if (isLogin)
            //{
            //    CommentMain.Log("登录用户：[" + CommentMain.playerName + "]");
            //}
        ...
        private void InitScrollItem(ref GameObject newGo, ref HorizontalLayoutGroup newGroup, ScrollRect scrollRect, List<CommentItem> items)
        {
            int maxCount = 15;
            for (int i = 0; i < items.Count; i++)
            {
                if (!isShowAll && i > maxCount)
                {
                    GameObject maxGo = CreateUI.NewText($"以下隐藏{items.Count - maxCount}条评论，点击展开。", new Vector2(445, 20));
                    maxGo.transform.SetParent(scrollRect.content, false);
                    Text maxText = maxGo.GetComponent<Text>();
                    maxText.color = new Color(1, 0.298f, 0.298f);
                    maxText.fontSize = 18;
                    Action clickHide = () =>
                    {
                        Action checkOpenHide = () =>
                        {
                            isShowAll = true;
                            InitUI();
                        };
                        g.ui.OpenUI<UICheckPopup>(UIType.CheckPopup).InitData(GameTool.LS("common_tishi"), "显示过多评论可能造成卡顿，是否继续？", 2, checkOpenHide);
                    };
                    maxGo.AddComponent<Button>().onClick.AddListener(clickHide);
                    break;
                }
        ...
        public void InitUser()
        {
            ...
            if (!string.IsNullOrWhiteSpace(CommentMain.playerName))
            {
                ...
                for (int i = 0; i < myData.Count; i++)
                {
                    ...
                    CommentMain.Log($"{i} >> 昵称：{item.e} 时间：{item.b} 好评：{item.c} 差评：{item.d} 内容：{item.a}");
            ...
            items.Sort((a, b) =>
            {
                ...
                switch (sort)
                {
                    ...
                    case 2:
                        ...
                        return (a.time) < (b.time) ? 1 : -1;
                    case 1:
                        if (a.good - a.bad * 1.5f == b.good - b.bad * 1.5f)
                            return 0;
        ...
        public string GetTypeTitle(int type, int target)
        {
            ...
            CommentMain.Log("未分类|" + targetType + "|" + targetId);
            return "未分类";
        }
    }
}
